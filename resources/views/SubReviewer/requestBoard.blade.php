...
<head>
    ...
    <title>SubReviewer Request Dashboard</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">{{$conference->acronym}} List of Submissions</h1>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                        <p>You {{$user->firstName}} {{$user->lastName}} are <strong>requested</strong> to Review Papers in an {{$track->Name}} in {{$conference->conferenceName}}</p>
                        <p>Track    : <strong>{{$track->Name}}</strong>   </p> 
                        <p>Conference    :   <strong>{{$conference->conferenceName}}</strong> </p> 
                        <p>Conference Starting Date    : <strong>{{$conference->startingDate}}</strong>   </p> 
                        <p>Conference Ending Date    :  <strong>{{$conference->endingDate}}</strong>  </p> 
                        <p>Conference Primary Aim    :  <strong>{{$conference->primaryAim}}</strong>  </p>
                        <p>Conference Secondry Aim    :    <strong>{{$conference->secondaryAim}}</strong> </p>
                        <?php $hashids = new Hashids\Hashids('',40); $roleId=$hashids->encode($role->id); ?>
 
                        <center><a  href="{{ route('subreviewerrejectOffer', $roleId) }}" title="Reject"><button type="button" class="btn btn-danger">Reject</button></a>
                        <a  href="{{ route('subrevieweracceptOffer' , $roleId) }}" title="Accept"><button type="button" class="btn btn-info">Accept</button></a></center>
